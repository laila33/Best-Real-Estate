...
function Discover_Properties() {
  const categories = [
    ...
    { id: 6, name: "Warehouse", count: 3, icon: "../warehouse.svg" },
  ];
  ...
  return (
    <div className="p-2 md:px-40 mt-20 flex flex-col gap-10">
      <h1 className="self-start text-4xl  dark:text-white text-[#102D47] font-bold">
        Discover Properties
      </h1>
      ...
      >
        {categories.map((item) => (
          <Link
            href={`/properties?type=${item.name}`}
            key={item.id}
            className="block"
          >
            <motion.div
            ...
            >
              ...
              <p className="text-2xl font-bold text-[#102D47] dark:text-white">
                {item.name}
              </p>
